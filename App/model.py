...
def newCatalog():
    ...
    catalog['Medium'] = mp.newMap(40,
                                 maptype='PROBING',
                                 loadfactor=0.5)
    
    catalog['Nationality'] = mp.newMap(40,
                                 maptype='PROBING',
                                 loadfactor=0.5)
    return catalog
...
def addMedium(catalog, book):
    lt.addLast(catalog["books"], book)
    mp.put(catalog["Medium"],book["goodreads_books_Nationality"], book)

def addNationality(catalog, book):
    lt.addLast(catalog["Nationality"], book)
    mp.put(catalog['Nationality'])
